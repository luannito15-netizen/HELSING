...
## Referência de controle — WORKING, REGISTRADA

A família MOBA mobile — Mobile Legends, LoL Wild Rift — recebeu entrada formal em [Decisions Log](DECISIONS_LOG.md) em 2026-08-15, na seção `WORKING`. Deixa de ser pendência. Nenhuma decisão `LOCKED` foi alterada; a consequência implementada continua sendo o facing seguir o movimento quando não há mira manual, atrás de `faceMovementWhenNotAiming`.

## Reconciliation gate — RESOLVIDO

`VISION / LOCKED ORDER RECONCILIATION — RESOLVED` em 2026-08-15. Adotada a divisão em dois gates formais, sem remover nenhum item do marco:

1. **`GATE A — ALUCARD PLAYABLE PRE-ALPHA 01`** — faltam Jackal, weapon swap e um poder;
2. **`GATE B — PRODUCT EXTRACTION GATE`** — o `P2 — Extraction Loop`.

Ordem oficial: A antes de B. Registro e justificativa em [Decisions Log](DECISIONS_LOG.md). É `WORKING` e reversível.
...
- [x] `DONE` — Joystick virtual esquerdo: automação, harness de dois pointers e smoke manual `PASS`; correção de ciclo de vida validada; `REAL DEVICE PASS` em 2026-08-15, já na forma dinâmica, com o stick reposicionado sob o dedo e aprovado como `perfeito` pelo Game Director.
...
- [x] `DONE` — Mira manual greybox por arrasto: automação, harness de dois pointers e smoke manual `PASS`; correção de ciclo de vida validada; `REAL DEVICE PASS` em 2026-08-15. Fica registrado como `TUNING / OPEN` o pedido do Game Director de melhorar a sensibilidade em ~6–7%, candidato a slider nas configurações do jogo e não a valor fixo.
- [ ] Implementar troca Casull/Jackal.
- [x] `DONE / TUNING OPEN` — Dash implementado em `DashController`: gatilho por arrasto, direção vinda do stick, distância `2.25`, cooldown `0,9 s`, suspende `PlayerMovement` por `0,18 s` e mantém gravidade. `REAL DEVICE PASS` em 2026-08-15. Sem custo de recurso — Sangue e Almas são economia e seguem `OPEN`.
...
- `PROJECT-WIDE ACTIONS — RESOLVIDO` em 2026-08-15. As Project-wide Actions foram repontadas do template do Unity 6 para `HelsingGameplay` pela API do Input System — `InputSystem.actions`, sem edição de YAML à mão — e o template `Assets/InputSystem_Actions.inputactions` foi removido do projeto. Antes da remoção foi verificado que nenhuma cena, prefab, asset ou `ProjectSettings` o referenciava. Com isso morre a terceira fonte de input não documentada que originou o `P3`: passa a existir um único asset de ações no projeto.
